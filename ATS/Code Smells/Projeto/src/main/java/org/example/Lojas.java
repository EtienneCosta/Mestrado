...
public class Lojas extends User implements Serializable{
    ...
    public List<Encomenda> getEmPreparacao (){
        return this.emPreparacao.stream().map(Encomenda::clone).collect(Collectors.toList());
    }

    public void setEmPreparacao (List<Encomenda> le) {
        this.emPreparacao = le.stream().map(Encomenda::clone).collect(Collectors.toList());
    }
