...
public class RegistoEncomendaLista implements Serializable{
    ...
    public List<RegistoEncomenda> getRegisto() {
        return this.registo.stream().map(RegistoEncomenda::clone).collect(Collectors.toList());
    }

    public void setRegisto(List<RegistoEncomenda> registo) {
        this.registo = registo.stream().map(RegistoEncomenda::clone).collect(Collectors.toList());
    }
